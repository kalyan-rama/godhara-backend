...
apiRouter.post('/auth/admin-otp-verify', authRateLimiter, async (req, res) => {
  ...
  // Simple, secure 2FA simulation: code is hardcoded or emailed to testing workspace
  const isDemoCode = code === '108108'; // Demo OTP code for testing ease
  if (!isDemoCode) {
    return res.status(400).json({ message: 'Invalid Admin 2-Factor Authentication passcode.' });
  }
