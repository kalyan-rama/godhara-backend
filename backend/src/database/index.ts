...
export async function ensureSchema() {
  ...
  try {
    ...
    // Add delivery charge columns if missing (migration for existing DBs without them)
    await client.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS "deliveryChargeTelangana" NUMERIC DEFAULT 70`).catch(() => {});
    await client.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS "deliveryChargeAP" NUMERIC DEFAULT 80`).catch(() => {});
    await client.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS "deliveryChargeOther" NUMERIC DEFAULT 100`).catch(() => {});
    await client.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS "freeDeliveryPincodes" TEXT DEFAULT '[]'`).catch(() => {});
    await client.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS "storeLocations" TEXT DEFAULT '[]'`).catch(() => {});
    await client.query(`ALTER TABLE settings ADD COLUMN IF NOT EXISTS "storeServicePincodes" TEXT DEFAULT '[]'`).catch(() => {});

    // Cart indexes for fast per-user lookups
    await client.query(`CREATE INDEX IF NOT EXISTS idx_carts_userid ON carts("userId")`).catch(() => {});
...
export async function reloadCache() {
  ...
  try {
    ...
    const ms = Date.now() - start;
    if (ms > 500) {
      console.warn(`⚠️  [SLOW QUERY] [reloadCache] took ${ms}ms (>500ms threshold) — Neon cold start likely`);
    }
  } catch (err) {
...
export const dbObj = {
  ...
  getCart(userId: string) {
    ...
    if (!cart) {
      // Create in-memory only — use targeted pgUpsertCart, NOT full writeData/flushToPostgres
      const newCart = { id: `cart-${Date.now()}`, userId, items: [], updatedAt: new Date().toISOString() };
      data.carts.push(newCart);
      cache = data; // update in-memory cache directly without triggering full flush
      pgUpsertCart(userId, newCart.id, [], newCart.updatedAt).catch(() => {});
      return newCart;
    }
    return cart;
  },
  saveCart(userId: string, items: any[]) {
    ...
    // Update in-memory cache directly — then fire targeted cart upsert only
    // This avoids re-flushing all users/products/orders/coupons on every cart change
    cache = data;
    pgUpsertCart(userId, cart.id, items, cart.updatedAt).catch(() => {});
    return cart;
  },
